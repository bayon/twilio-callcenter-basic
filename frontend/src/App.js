...
function App() {
  ...
  useEffect(() => {
    ...
    socket.client.on("call-new", ({ data: { CallSid, CallStatus, From } }) => {
      console.log('frontend: call-new:....')
      console.log('CallSid, CallStatus, From ', CallSid, CallStatus, From )
      setCalls((draft) => {
        const index = draft.calls.findIndex(call => call.CallSid === CallSid)
        //prevent having duplicate  calls display on browser
        if(index === -1){
